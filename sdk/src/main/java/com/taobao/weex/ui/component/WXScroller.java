...
public class WXScroller extends WXVContainer<ViewGroup> implements WXScrollViewListener, Scrollable {
    ...
    private Object mScrollListener;
    ...
    protected ViewGroup initComponentHostView(@NonNull Context context) {
        ...
        if (getDomObject() == null || getDomObject().getAttrs().isEmpty()) {
            ...
            mScrollListener = getDomObject().getAttrs().get("scrolllistener");
        }
    ...
    // TODO: 2016/11/3 增加了滑动事件监听
    @Override
    public void onScrollChanged(WXScrollView scrollView, int x, int y,
                                int oldx, int oldy) {
        if (mScrollListener != null) {
            try {
                boolean can = Boolean.parseBoolean(mScrollListener.toString());
                if (can){
                    Map<String, Object> params = new HashMap<>();
                    params.put("cx", x);
                    params.put("cy", y);
                    getInstance().fireEvent(getRef(), Constants.Event.LIST_SCROLL, params);
                }
            }catch (Exception e){
                e.printStackTrace();
            }
